  ...
  useEffect(() => {
    ...
    if (sharer && sharer.socketId) {
      ...
      if (!pcsRef.current[sharerSocketId]) {
        console.log(`подключаемся к стриму @${sharer.username}...`);
        ...
        pc.ontrack = (event) => {
          console.log("получен видео-трек стрима!", event.streams[0]);
  ...
  useEffect(() => {
    ...
    const handleSignal = async (data) => {
      ...
      if (signalData.type === 'offer') {
        ...
        console.log(`получен offer от зрителя (id: ${senderSocketId})`);
